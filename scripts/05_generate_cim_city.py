...
def strip_mesh(
    ...
    if dash_length is None:
        ...
        while distance < line.length:
            ...
            distance += period
    spans = subtract_blocked_spans(spans, blocked_distances or [], blocked_clearance, float(line.length))
...
def junction_approach_offsets_for_row(row: pd.Series, rule: Any) -> tuple[float, float]:
    ...
    crosswalk_offset = max(6.0, min(34.0, clearance + CROSSWALK_BAND_LENGTH_M * 0.5))
...
def rounded_junction_polygon(
    ...
    for road_idx, distance_hint in members:
        if road_idx in seen or road_idx not in prepared_roads.index:
            continue
        seen.add(road_idx)
        ...
        if width <= 0.1:
            continue
        widths.append(width)
        distance = max(0.0, min(float(line.length), float(distance_hint)))
        if line.distance(point) <= road_gen.junction_connection_tolerance():
            distance = float(line.project(point))
        throat = max(
            JUNCTION_PATCH_MIN_THROAT_M,
            min(JUNCTION_PATCH_MAX_THROAT_M, width * 0.75 + 6.0),
        )
        start = max(0.0, distance - throat)
        end = min(float(line.length), distance + throat)
        if end - start <= 0.1:
            continue
        try:
    ...
    core_radius = max(max(widths, default=0.0) * 0.45, 4.0)
    parts.append(point.buffer(core_radius, resolution=18))
    try:
        geom = unary_union(parts)
        smooth = JUNCTION_PATCH_SMOOTH_M
...
def build_rounded_junction_surface_meshes(
    ...
    for idx, bucket in enumerate(junction_point_buckets(prepared_roads)):
        geom = rounded_junction_polygon(prepared_roads, rules, bucket["point"], bucket["members"])
        if geom is None or geom.is_empty:
            continue
...
def build_road_surface_meshes(roads: gpd.GeoDataFrame) -> dict[str, trimesh.Trimesh]:
    ...
    junction_surface_meshes = build_rounded_junction_surface_meshes(prepared_roads, rules)
        ...
        for line_idx, line in enumerate(iter_lines(row.geometry)):
            ...
            for component_idx, (component, left_offset, right_offset) in enumerate(line_spans):
                ...
                mesh = swept_band_mesh(
                    ...
                    left_offset,
                    right_offset,
                    f"{layer_name}_{road_idx}_{line_idx}_{component_idx}",
                    color,
            ...
            add_center_markings_for_adjacent_carriageways(row, line, rule, line_spans, yellow_marking_meshes)
            add_component_curbs(row, line, rule, line_spans, curb_meshes)
...
def junction_arm_records(
    ...
    for road_idx, distance_hint in members:
        ...
        if line is None or line.is_empty or not isinstance(line, LineString):
            continue
        rule = road_gen.get_road_rule(row, rules)
        node_distance = float(distance_hint)
        ...
        records.append(
            {
                ...
                "surface_strategy": "rounded_conflict_area_with_socket_semantics",
                "center_local": {
...
def build_city_junction_semantic(prepared_roads: gpd.GeoDataFrame, origin: tuple[float, float]) -> dict[str, Any]:
    ...
    return {
        ...
        "semantic_level": "topology_node_with_arms_and_movements",
        "objects": records,
        "summary": {
...
def build_city_road_model_score(prepared_roads: gpd.GeoDataFrame, road_meshes: dict[str, trimesh.Trimesh]) -> dict[str, Any]:
    ...
    if roads_with_junction_clearance and "Junction_Surface" in present_layers:
        findings.append("Rounded junction surfaces are generated and lane markings are cleared inside junction approach zones.")
...
def build_city_junction_score(
    ...
    if {"Junction_Surface", "Crosswalk", "Stop_Line"} <= present_layers:
        findings.append("The visible junction model now includes conflict-area asphalt, pedestrian crossings, and stop control markings.")
