...
"""Launch Blender to export the generated CIM city OBJ files as materialized FBX."""
...
BLENDER_EXPORT_SCRIPT = ROOT / "scripts" / "06_export_cim_city_fbx_blender.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[1/1] Exporting materialized FBX with Blender: {blender}", flush=True)
    subprocess.run(command, cwd=ROOT, check=True)
    print(f"FBX output: {ROOT / 'output' / 'fbx' / 'cim_city.fbx'}", flush=True)
    return 0
