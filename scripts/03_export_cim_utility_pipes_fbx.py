...
"""Launch Blender to export generated CIM utility pipe OBJ modules as FBX."""
...
BLENDER_EXPORT_SCRIPT = ROOT / "scripts" / "04_export_cim_utility_pipes_fbx_blender.py"


def utility_fbx_path(level: str) -> Path:
    return ROOT / "output" / "fbx" / "modules" / level / "city_utility_pipes.fbx"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="both",
        help="Utility generation detail level to export. Use 'both' to export cim3 and cim4. Default: both.",
    )
    ...
    for level in levels:
        ...
        print(f"[1/1] Exporting {level} utility FBX with Blender: {blender}", flush=True)
        subprocess.run(command, cwd=ROOT, check=True, env=env)
        print(f"Utility FBX output: {utility_fbx_path(level)}", flush=True)
    return 0
