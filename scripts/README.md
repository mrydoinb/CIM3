...
current hybrid subway tunnel export. It reuses the compact OBJ generated by
`02_generate_cim_subway_tunnels.py`, infers line filters from
`subway_tunnels_semantic.json`, and adds the sample-derived small components to
the FBX. Use `--procedural-only` only when you intentionally want the old
direct OBJ-to-FBX path.
