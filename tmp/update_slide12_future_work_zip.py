...
def find_base_ppt() -> Path:
    ...
    if not candidates:
        raise SystemExit("No base replaced PPTX found in Downloads.")
...
def main() -> None:
    ...
    with ZipFile(src, "r") as zin:
        slide_xml = zin.read("ppt/slides/slide12.xml").decode("utf-8")
        for old, new in REPLACEMENTS.items():
            if old not in slide_xml:
        ...
        with ZipFile(out, "w", ZIP_DEFLATED) as zout:
            for info in zin.infolist():
                if info.filename == "ppt/slides/slide12.xml":
                    zout.writestr(info, slide_xml.encode("utf-8"))
                else:
